...
public class EnemyB : MonoBehaviour {
    ...
    private float attackRadius=1.31f;
    public float speed;
    ...
    IEnumerator Attack(float seconds)
    {
        ...
        if (target != initialPosition && Golpe != null)
        {
    ...
    public void Attacked()
    {
        ...
        //Comprovacion con reduccion de vida, si al enemigo ya no tiene vidas, se destuira.
        if (--hp <= 0)
        {

            if (Player.bandera <= 2)
            {
                NPC.reputation = NPC.reputation + 3;

                Player.bandera++;
            }
            else if (Player.bandera == 3)
            {
                NPC.reputation = NPC.reputation + 6;
                Player.bandera++;
            }
            else if (Player.bandera == 4)
            {
                NPC.reputation = NPC.reputation + 20;
                Player.bandera++;
            }
            else if (Player.bandera == 5)
            {
                NPC.reputation = NPC.reputation + 20;
                Player.bandera++;
            }
